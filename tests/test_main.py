...
class MainTestCase(unittest.TestCase):
    ...
    def test_main_marks_claude_rejected_project_without_sending_project_notification(self):
        item = {
    ...
    def test_email_send_uses_starttls_for_587_and_sends_message(self):
        fake_server = mock.Mock()
        fake_smtp_cls = mock.Mock(return_value=fake_server)
        fake_server.__enter__ = mock.Mock(return_value=fake_server)
        fake_server.__exit__ = mock.Mock(return_value=None)
        ...
        with mock.patch.dict(os.environ, env, clear=True), \
                mock.patch.object(self.main.smtplib, "SMTP", fake_smtp_cls), \
                mock.patch.object(self.main.smtplib, "SMTP_SSL") as smtp_ssl:
            ok = self.main.email_send("Новый заказ")

        self.assertTrue(ok)
        fake_smtp_cls.assert_called_once_with("smtp.example.test", 587, timeout=30)
        fake_server.starttls.assert_called_once_with()
        fake_server.login.assert_called_once_with("from@example.test", "secret")
        fake_server.sendmail.assert_called_once()
        self.assertEqual(fake_server.sendmail.call_args.args[0], "from@example.test")
        self.assertEqual(fake_server.sendmail.call_args.args[1], "to@example.test")
